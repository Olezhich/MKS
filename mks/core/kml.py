...
if __name__ == "__main__":
    ...
    # Трек 1: Красная линия с разрывом посередине
    track1 = np.array(
        [
            [37.6173, 55.7558],  # Москва
            [37.6200, 55.7600],
            [np.nan, np.nan],  # РАЗРЫВ
            [37.6300, 55.7700],
            [37.6350, 55.7750],
        ]
    )

    # Трек 2: Синяя линия с разрывом в самом начале и в конце
    track2 = np.array(
        [
            [np.nan, np.nan],  # РАЗРЫВ
            [30.3241, 59.9311],  # Санкт-Петербург
            [30.3300, 59.9350],
            [30.3350, 59.9400],
            [np.nan, np.nan],  # РАЗРЫВ
        ]
    ...
    colors = ["#FF0000", "#0000FF"]  # Красный и Синий

    # Создаем KML
    create_kml_from_tracks(tracks, colors, output_file="my_tracks.kml")
