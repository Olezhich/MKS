...
def earth_intersect(
    station_current: Station, cam_in_gcs_batch: np.ndarray
) -> np.ndarray:
    """Вычисляет точку пересечения вектора камеры с землей в ГСК"""
    res = []
    i = 0
    for cam_in_gcs in cam_in_gcs_batch:
        ...
        mks_x, mks_y, mks_z = station_current.position[i]
        i += 1
        a_semi = 6378.137
        ...
        res.append([mks_x + t * d_x, mks_y + t * d_y, mks_z + t * d_z])
    return np.array(res)
...
def calculate_cam_points(cam: Camera, mount: Mount, station: Station) -> Any:
    ...
    # Переводим в ГСК
    M = station.get_gcs_basis_matrix()
    cam_in_GCS = np.einsum("nij,j->ni", M, cam_in_OCS)

    # Ищем точку пересечения вектора камеры и сферы земли в ГСК
    cam_dot_GCS = earth_intersect(station, cam_in_GCS)
